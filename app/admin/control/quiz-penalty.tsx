...
export default function QuizPenaltyScreen() {
  ...
  const [lowSeconds, setLowSeconds] = useState("30");
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      const config =
        (await adminApi.getQuizPenaltyConfig()) as QuizPenaltyConfigDTO;
      setLowSeconds(String(config.lowPenaltySeconds));
  ...
  const handleSave = useCallback(async () => {
    ...
    const lowValue = parseSeconds(lowSeconds);
    const minorValue = parseSeconds(minorSeconds);
    const majorValue = parseSeconds(majorSeconds);

    if (lowValue === null || minorValue === null || majorValue === null) {
      showAlert({
        title: "Ungültige Eingabe",
        message: "Bitte gebt für alle Felder eine nicht-negative Zahl ein.",
      });
      return;
    }

    if (minorValue < lowValue) {
      showAlert({
        title: "Prüft die Werte",
        message:
          "Die Strafe für >50% falsche Antworten sollte mindestens so hoch sein wie für >25%.",
      });
      return;
    }
    ...
    if (majorValue < minorValue) {
      showAlert({
        title: "Prüft die Werte",
        message:
          "Die Strafe für >75% falsche Antworten sollte mindestens so hoch sein wie für >50%.",
      });
      return;
    }
    ...
    try {
      ...
      const message =
        err instanceof ApiError ? err.message : "Speichern nicht möglich";
      showError(message);
    } finally {
      setSaving(false);
    }
  }, [ensureSession, lowSeconds, minorSeconds, majorSeconds, parseSeconds]);
    ...
    if (loading) {
      return (
        <View style={styles.centerContent}>
          ...
          <ThemedText style={{ marginTop: 12, color: theme.textMuted }}>
    ...
    if (error) {
      return (
        <View style={styles.centerContent}>
          ...
          />
          <ThemedText
            style={{ marginTop: 12, color: theme.danger, textAlign: "center" }}
          >
            {error}
          </ThemedText>
          <TouchableOpacity
      ...
      >
        <View style={styles.headerRow}>
          <View style={{ flex: 1, gap: 4 }}>
            ...
            <ThemedText style={{ color: theme.textMuted, fontSize: 13 }}>
              Legt fest, wie viele Sekunden bei vielen falschen Antworten im
              Quiz addiert werden.
            </ThemedText>
          </View>
          <TouchableOpacity
        ...
        <View style={styles.formGroup}>
          <ThemedText style={styles.inputLabel}>
            Mehr als 25% falsch beantwortet
          </ThemedText>
          <View style={styles.inputRow}>
            <TextInput
              value={lowSeconds}
              onChangeText={(value) => {
                setLowSeconds(value.replace(/[^0-9]/g, ""));
                setSuccessMessage(null);
              }}
              inputMode="numeric"
        ...
        <View style={styles.formGroup}>
          <ThemedText style={styles.inputLabel}>
            Mehr als 50% falsch beantwortet
          </ThemedText>
          <View style={styles.inputRow}>
            <TextInput
              value={minorSeconds}
        ...
        <View style={styles.formGroup}>
          <ThemedText style={styles.inputLabel}>
            Mehr als 75% falsch beantwortet
          </ThemedText>
          <View style={styles.inputRow}>
            <TextInput
              value={majorSeconds}
